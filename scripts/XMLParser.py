...
class XMLParser:
    ...
    def stream(self,indata,header=False, bracket=False, output_sseq=False, output_ranges=False):
        ...
        if output_ranges and not self.ranges_done:
                result += "\t"
                result += "\t".join('qfrom qto qseqlen sfrom sto sseqlen'.split())
                self.ranges_done = True
        result += "\n"
        if bracket: result += "#}\n"
        iline=0
        metadata=[]
        ...
        for line in indata:
                ...
                if line[0]=='>':
                        ...
                        if self.inquery==1:
                                ...
                                qpid = self.qpid or str(self.iquery)
                                result += "\t".join([str(self.iquery),"0",qpid,pid,qcov,scov,str(self.bits),str(self.pide),str(self.lali),desc,species,seq,self.vote,genename,str(self.evalue)])
                                if output_ranges:
                                        result += "\t"
    ...
    def feed_line(self, line, header=False, bracket=False,
              output_sseq=False, output_ranges=False):
        """
        Incremental streaming interface.
        Returns (result, metadata) ONLY when a full </QUERY> is seen.
        ...
        result, metadata = self.stream(
            chunk,
            header=header,
            bracket=bracket,
            output_sseq=output_sseq,
