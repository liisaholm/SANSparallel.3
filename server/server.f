...
c=======================================================================
c
        module socket_interface
        use iso_c_binding
        implicit none

        interface
            subroutine check_sockets_briefly(l_sd, m_sd, m_set) bind(C,
     $            name="check_sockets_briefly")
            import :: c_int
            integer(c_int), intent(in) :: l_sd
            integer(c_int), intent(inout) :: m_sd
           ! We treat the fd_set as an opaque byte array to match C's
           ! memory layout
            integer(c_int), intent(inout) :: m_set(*) 
            end subroutine check_sockets_briefly
        end interface
        end module socket_interface
...
c
        ...
        implicit none
        integer nprot_tot,qseqlen
        integer*8 naa_tot

        hsplen=lnkappa+log(qseqlen+0.1)+log(naa_tot/1000.0)+
     $          log(1000.0d0/entropy)
        neff=max(1,qseqlen-hsplen)
        meff = naa_tot - int(hsplen,8) * int(nprot_tot,8)
        neffmeff=neff*meff
!        write(*,*) '#altschul',qseqlen,hsplen,neff,meff,neffmeff,
!     $          naa_tot,nprot_tot
...
c====================================================================
c
        subroutine blosum2pssm(qseqlen,qtxt,blosum,PSSM,alfmap)
        implicit none
        integer qseqlen
        integer blosum(0:20,0:20),PSSM(0:20,qseqlen)
        integer alfmap(128)
...
c
        ...
        integer, parameter :: maxitem=20000000 ! must match MAXDIAG in
c                                                module sans
     ...
     $          linklist_value,linklist_first,M,nprot)
        ...
        if(nkey.ge.maxitem) then
            write(*,*) '#WARNING: linkhash full, nkey=',nkey,
     $          ' nshort=',nshort,' increase maxitem'
            return
        end if
...
c
        ...
        subroutine seedkeys(qseqlen,qprot,kmer,qtxt,PSSM,alfmap)
        implicit none
        integer qprot,kmer,alfmap(128)
        integer qres,score,i,ix(maxres),qseqlen
        logical ok(maxres)
...
c----------------------------------------------------------------------
c
        subroutine get_keymap(qseqlen,qprot,scorecutoff,kmer,alfmap,
     $          MPI_COMM_SANS,PSSM,qtxt)
        implicit none
        include 'mpif.h'
        integer offset,x,i,j,k,keyix,aaix,ix,slice,kmer,alfmap(128)
        integer scorecutoff,ierr,MPI_COMM_SANS
        integer qseqlen,PSSM(0:20,maxres)
        integer ixout,scoreout,mutpos,q,qpos,qprot,score
        integer*1 qtxt(qseqlen)

        !write(*,*) '#get_keymap',qprot,scorecutoff,kmer,qseqlen
        ! unmutated query words
        heap_n=0
        call seedkeys(qseqlen,qprot,kmer,qtxt,PSSM,alfmap)
        ...
        do while(heap_n.gt.0)
                ...
                ! mutate words
               call pointmutate(ix,scorecutoff,kmer,PSSM,qprot,qseqlen)
        end do
        ! keymap is result
...
c----------------------------------------------------------------------
c
        subroutine pointmutate(ix,scorecutoff,kmer,PSSM,qprot,qseqlen)
        implicit none
...
c
     ...
     $          naa_tot,nprot_tot
        ...
        implicit none
        integer qseqlen,naccept,nreject ! module-level qseqlen
        ...
        integer, parameter :: MAXDIAG=20000000 ! 20 M
...
c
     ...
     $          rank,HX,W,MIN_SUMMA)
        ...
        do j=1,qseqlen ! test qpos in lexical order
                ...
                if(m.lt.1) m=1
                if(m.ge.naa) m=naa-1
                if(.false..and.qres.lt.qseqlen-20) write(*,*)
     $  '#bsearch',j,qres,m,rank,(char(qtxt(qres+i)),i=1,20),' ',
     $  (char(TXT(p)),p=ptr(SAP(m))+SRES(m)+1,ptr(SAP(m))+SRES(m)+20)
                m_prev=m ! to be safe
...
c
c=====================================================================
c     do_all: main server loop
c     Receives blocks of queries via MPI from socket rank (Rank 0),
c     processes them sequentially, sends results back one by one.
c
c     MPI protocol with server_c.c:
c       Rank 0 -> all ranks: MPI_Isend(header[2], tag=0)
c                            header(1) = block_len (bytes)
c                            header(2) = n_queries (count)
c       Rank 0 -> Rank 1:    MPI_Send(block, block_len, tag=0)
c       Rank 1 -> Rank 0:    MPI_Send(res_len, 1, tag=0)  } repeated
c       Rank 1 -> Rank 0:    MPI_Send(result, res_len, tag=0) 
c                                ! n_queries times
c
c     Shutdown sentinel: header(1)=0, header(2)=0
c=====================================================================

        subroutine do_all(socket,dbversion,programtag,nucl)
        use socket_interface
        !use sans
        implicit none
        include 'mpif.h'

        integer socket,rank,ierr,ok,nproc
        double precision starttime,t,t0
        integer header(2),block_len,n_queries
        integer request,stat(MPI_STATUS_SIZE)
        logical lterminate,nucl
        integer, parameter :: secs=0, usecs=500000
        integer, parameter :: TIMEOUT=60
        character(len=256) dbversion,programtag
        integer uid
        double precision dt

        ! MAXBLOCK must hold largest possible chunk:
        ! 100 queries x 64000 bytes = 6.4MB; use 7MB for safety
        ...
        call MPI_COMM_SIZE(MPI_COMM_SANS, nproc_sans, ierr)
...
!     $                  ' n_queries=',n_queries
                ...
                !---------------------------------------------------------
                if(block_len.le.0) then
                        lterminate=.true.
                        exit
                end if

                ! Sanity check
                if(block_len.gt.MAXBLOCK) then
                  write(*,*) '# ERROR: block_len exceeds MAXBLOCK',
     $                block_len,MAXBLOCK
                  lterminate=.true.
                  exit
                end if

                if(n_queries.le.0) then
                        write(*,*) '# WARNING: n_queries=0, skipping'
                        cycle
                end if

                !---------------------------------------------------------
                ! Rank 0 of MPI_COMM_SANS (=Rank 1 of MPI_COMM_WORLD)
                ! receives the full block string from socket rank
                !---------------------------------------------------------
                if(rank.eq.0) then
                    call MPI_RECV(block,block_len,
     $                  MPI_CHARACTER,socket,MPI_ANY_TAG,
     $                  MPI_COMM_WORLD,stat,ierr)
!                    write(*,*) "Received ",block_len, block(1:block_len)
                end if

                !---------------------------------------------------------
                ! Broadcast entire block to all workers
                !---------------------------------------------------------
                call MPI_BCAST(block,block_len,MPI_CHARACTER,
     $                  0,MPI_COMM_SANS,ierr)

                !---------------------------------------------------------
                ! Process all queries in this block sequentially
                !---------------------------------------------------------
                call process_block(block,block_len,n_queries,
     $                  socket,dbversion,programtag,uid,nproc_sans)
        ...
        !-----------------------------------------------------------------
        t=MPI_WTIME()
        write(*,*) '# do_all: rank=',rank,' uid=',uid,
     $          ' queries in ',t-starttime,' seconds'

        end subroutine do_all

c=====================================================================
c     process_block: parse block of N query strings and process each
c
c     Block format (from server_c.c):
c       query1_string</QUERY>\n
c       query2_string</QUERY>\n
c       ...
c       queryN_string</QUERY>\n
c
c     Each query string is a complete SANSparallel query line:
c       qprot H HX W MIN_SUMMA MINKEYSCORE evalue R votelist protocol
c       "sequence" "header" </QUERY>
c
c     Results are sent back to socket rank one at a time via
c     send_result() after each do_one() call, so the client
c     receives results as they are computed rather than waiting
c     for the entire block.
c=====================================================================

        subroutine process_block(block,block_len,n_queries,
     $          socket,dbversion,programtag,uid,nproc_sans)
        !use sans
        implicit none
        include 'mpif.h'
        ...
        character(len=*) block
        integer block_len,n_queries,socket,uid,nproc_sans
        character(len=256) dbversion,programtag

        integer iq,pos,next_pos,qlen,rank,ierr,qprot
        character(len=MAXRES) qstring
        character(len=9), parameter :: MARKER='</QUERY>'//char(10)
        integer, parameter :: MARKLEN=9
        ...
        do iq=1,n_queries
                ...
                if(pos.gt.block_len) exit

                ! index() returns the position relative to the start of
                ! the substring (1-based)
                next_pos=index(block(pos:block_len),MARKER)

                if(next_pos.le.0) then
                    write(*,*) '# WARNING: no </QUERY> at query', iq
                    exit
                end if

                ! next_pos is the start of the marker relative to pos.
                ! So the query length is next_pos - 1.
                qlen = next_pos - 1

                ! Extract query string using the current pos
                qstring=' '
                if (qlen.gt.0) then
                    if(qlen.gt.MAXRES) qlen=MAXRES
                    qstring(1:qlen) = block(pos : pos + qlen - 1)
                end if

!                if(rank.eq.0) write(*,*) '# process_block query', iq, 
!     $              'qlen=', qlen

                ! --- Process Query ---
                uid = uid + 1
                call do_one(socket,qstring,rank,nproc_sans,uid,
     $               dbversion,programtag,qlen)

                ! --- Advance Position ---
                ! Move pos to the start of the character AFTER the
                ! </QUERY>\n
                pos = pos + (next_pos + MARKLEN - 1)
...
c--------------------------------------------------------------------
c
        subroutine voting(qseq,lreverse,H,HX,W,MIN_SUMMA,
     $          rank,vote_cutoff)
        implicit none
        include 'mpif.h'
...
c        
     ...
     $          programtag,qlen_in)
        ...
        ! FIRST executable statement - reset all per-query state
        call init_query()
...
!     $          evalue_cutoff
        ! vot!e cutoff; naccept entries saved in diaglist(3,:)
        call voting(qseq,.false.,votelist_size,HX,W,
     $          MIN_SUMMA,rank,vote_cutoff)
!        if(rank.eq.0) write(*,*)
!     $          '#voting time',MPI_WTIME()-t,vote_cutoff,naccept
        ...
        call blosum2pssm(qseqlen,qtxt,blosum,PSSM,alfmap) 
         ! call keymap sequentially; do calculation on all nodes
         if(protocol.ge.1) then
!                t=MPI_WTIME()
                call get_keymap(qseqlen,uid,MINKEYSCORE,4,alfmap,
     $                  MPI_COMM_SANS,PSSM,qtxt)
...
!     $          heap_n,score_cutoff_prev
           do while(heap_n.gt.0)
                ntest=ntest+1
                summa=-1
                call align_next_sbjct(evalue_cutoff,
...
c
     ...
     $         bitscore,evalue,diagonal,line,offset,qfrom,qto,sfrom,sto)
        ...
        sseqlen=min(int(ptr(sprot+1)-ptr(sprot),4), MAXRES)
     ...
     $          '>',char(10)
                ...
                write(a,
     $  '(a12,i8,a6,i8,a6,f6.2,a6,i6,a6,f12.1,a8,e16.8,2(a6,i8),'//
...
c 
       ...
       sres=0
       sseqlen=int(ptr(sprot+1)-ptr(sprot)-1, 4)
       spos=ptr(sprot)
       do sres=1,sseqlen
                spos=spos+1
                s(sres)=alfmap(TXT(spos))
...
c
        ...
        if(spos_m.lt.1.or.spos_m.gt.naa) return
        if(spos_l.lt.1) return
        if(spos_r.gt.naa) return
        l=lcp(qres,spos_m)+1
        q=qtxt(qres+l)
        s=TXT(spos_m+l)
...
c
c       diagonal=qres-sres  (sres is LOCAL 1..sseqlen)
c
     ...
     $          bestnide,bestlali,bestqfrom,bestqto,bestsfrom,beststo)
        ...
        integer lspos  ! local spos = spos - s0
        ...
        ! get best trace
        qpos=1
        ! sleft, srite now in GLOBAL coordinates
        sleft=ptr(sprot)            ! global left boundary (exclusive)
        srite=ptr(sprot+1)-1        ! global right boundary (exclusive)
        ! initialize ARRAYS
        delete=0
        match=0
        insert=0
        ...
        sfrom_insert=0
        sfrom_delete=0
        ! fill local stxt and s arrays using local index i
        i=0
     ...
     $                                  sfrom_insert(prev,testdiag)
                        ...
                        if(qfrom_match(curr,testdiag).eq.0) then
                                ...
                                sfrom_match(curr,testdiag)=lspos
                        end if
                        dat(1)=match(curr,testdiag-1)+GAPOPEN
     ...
     $                                  sfrom_insert(curr,testdiag-1)
                        ...
                        dat(1)=match(prev,testdiag+1)+GAPOPEN
...
c
        ...
        subroutine init_query()
        ! Purpose: fully reset all per-query state in module sans and module
        ! myhash
        ! Call it as the FIRST executable statement in do_one(), before any
        ! early-return guards like 'if(qseqlen.le.7) goto 99'.
        implicit none
        include 'mpif.h'
        integer i, ierr

        ! --- module sans: heap state ---
        heap_n=0
!        heap_v=0
!        heap_input=0
!        heap_diag=0
...
!        heap_vote=0
        ...
        ! --- module sans: diaglist ---
        naccept=0
        nreject=0
        ! diaglist itself need not be zeroed since it is
        ! written before being read, up to naccept entries

        ! --- module sans: vote histogram ---
        ! vote_hist is local to voting()/local_votes() - OK

        ! --- module myhash: flush linklist_first for all
        !     entries registered in shortlist ---
        do i=1,nshort
                linklist_first(shortlist(i))=0
        end do
        ! reset hash counters
        nkey=0
        nshort=0
        ! linklist_next and linklist_key need not be zeroed:
        ! they are only accessed via valid linklist_first chains
        ! which have just been cleared above

        ! --- module keyspace: heap ---
        ! heap_n in keyspace is a separate variable from heap_n in sans
        ! it is reset at the start of get_keymap() - OK

        ! --- PSSM: set to zero so stale columns beyond qseqlen
        !     cannot affect tube_dp if qseqlen changes ---
!        PSSM=0

        ! --- qtxt: zero beyond current qseqlen is handled by
        !     make_qtxt_QSA which fills exactly qseqlen positions.
        !     Zero the rest to be safe ---
!        qtxt=0

        end subroutine init_query
...
c
        ...
        s=SRES(lex)
        spos=ptr(sprot)+s
        if(spos.gt.0.and.spos.lt.naa-20) write(*,*)
     $  '#SAP',lex,sprot,s,spos,(char(TXT(p)),p=spos-1,spos+20)
